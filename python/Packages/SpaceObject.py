class SpaceObject:  # extends ArravObject
    DEF_SPACE_OBJECT_PERIMERER: int = 25000
    id: int
    race: str = "Неопределено"
    _iCost: int = 0
    QCount: int = 0
    _nDisplaySize: float = 0
    # _sSelection:     
    # _sHoverSelection:     
    _iSelectionsChange: float = 1
    _nClassScale: float = 1
    _nScale: float = 1
    selectionColor: None = 16777215  # uint
    landable: bool = True
    allowSelection: bool
    _iLastClicked: int

    def __init__(self, param1: bool = True):
        self.allowSelection = param1
    ...
    # return 0
    #
    def typeOfObject(self) -> int:
        if self is Ship:
            return ObjectToReachType.SHIP
        if self is Planet:
            return ObjectToReachType.PLANET
        if self is Item:
            return ObjectToReachType.ITEM
        if self is AsteroidBelt or self is Portal or self is RepositoryStation or self is Hive:
            return ObjectToReachType.STATIC_SPACE_OBJECT
        if self is Asteroid:
            return ObjectToReachType.ASTEROID
        return 0
    #
    # displaySize() -> float:
    #
