...
class Inventory(B_Inventory):
    ...
    def use_weapon(self, ItemClass):
        ...
        self.ship[Keys.cpu_used] += ItemClass.cpu
    ...
    def drop_item(self, data):
        for item_ in self.inventory:
            if item_.guid == data[Keys.guid]:
                item_.drop(data[Keys.wear])
                break

    def device_clicked(self, data):
        if self.ship["device_slots"] > self.cnt_active_device:
            for item_ in self.activeDevices:
                if data[Keys.guid] == item_.guid:
                    item_.clicked(data)
                    break
